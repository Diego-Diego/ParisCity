...
function KeyboardInput(TextEntry, ExampleText, MaxStringLenght)
 ...
 -- MENU
 local mainMenu = RageUI.CreateMenu("", "MENU Norauto") 
     ...
     if open then 
         ...
         Citizen.CreateThread(function()
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous ne pouvez pas faire cette action depuis un véhicule", 'CHAR_CARSITE3', 7)
				return
			end
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Réparation ~h~~g~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			end
                    end
                   })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Nettoyage ~h~~g~Terminer ", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
			...
			if IsPedSittingInAnyVehicle(playerPed) then
                ...
                ClearPedTasks(playerPed)
                Citizen.Wait(2500)
                ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Mise en fourrière ~h~~g~Terminer", 'CHAR_CARSITE3', 7)
                ESX.Game.DeleteVehicle(vehicle)
				else
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous devez être sur la place conducteur !", 'CHAR_CARSITE3', 7)
				end
			else
				local vehicle = ESX.Game.GetVehicleInDirection()
				...
				if DoesEntityExist(vehicle) then
					...
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous devez être prêt d'un véhicule pour commencer la mise en fourrière", 'CHAR_CARSITE3', 7)
				end
			end
                    end 
                })
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Crochetage ~h~~g~Terminer", 'CHAR_CARSITE3', 7)
					isBusy = false
				end)
			else
				ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Aucun véhicule à proximité", 'CHAR_CARSITE3', 7)
						...
						if targetVehicle ~= 0 then
							if not IsPedInAnyVehicle(playerPed, true) then
								if vehicle ~= targetVehicle then
									...
									ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous venez d'attacher le véhicule avec ~h~~g~succés !", 'CHAR_CARSITE3', 7)
									...
									ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous ne pouvez pas attacher votre ~h~~r~propre véhicule de dépannage !", 'CHAR_CARSITE3', 7)
								end
							end
						else
							ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Il n'y a pas de véhicule ~h~~r~attacher !", 'CHAR_CARSITE3', 7)
						end
					else
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Ce n'est pas le bon ~h~~r~véhicule !", 'CHAR_CARSITE3', 7)
								end
	
							else
								ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous n'etes pas au bon endroit ~h~~r~pour faire cela !", 'CHAR_CARSITE3', 7)
							end
						end
	
						CurrentlyTowedVehicle = nil
						ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Véhicule détacher avec ~g~~h~succès !", 'CHAR_CARSITE3', 7)
					end
				else
					ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous devez avoir une ~r~~h~dépanneuse à plateau pour faire cela !", 'CHAR_CARSITE3', 7)
				end
                    end
                })
            end)
                ...
                RageUI.IsVisible(annoncemenu, function()
                    ...
                    RageUI.Button("Annonce [~g~OUVERTURE~s~]", nil, {RightLabel = "→→"}, true, {
                       ...
                       RageUI.Button("Annonce [~g~RECRUTEMENT~s~]", nil, {RightLabel = "→→"}, true, {
                        onSelected = function()
                    ...
                    RageUI.Button("Message [~g~Employer~s~]",  nil, {RightLabel = "→→"}, true , {
                        onSelected = function()
                            local message = KeyboardInput('Veuillez mettre le messsage à envoyer', '', 500)
         ...
         while open do 
            RageUI.IsVisible(mainMenu6,function() 
              ...
              RageUI.Separator(VarColor.."↓ Véhicules ↓")
                ...
                for k,v in pairs(Config.VehiculeBenny) do
                RageUI.Button(v.buttoname, nil, {RightLabel = "→→"}, true , {
                    onSelected = function()
                        ...
                        ESX.ShowNotification("~g~Norauto\n~r~Point de spawn bloquée")
                        else
                        ...
                        SetVehicleNumberPlateText(ambuveh, "Norauto"..math.random(50, 999))
...
Citizen.CreateThread(function()
                ...
                RageUI.IsVisible(mainMenuFrabrication,function() 
                  ...
                  RageUI.Button("Fabriquer un kit de réparation", 'Prix du matériel néceissaire : ~y~10€', {RightLabel = "→→"}, true , {
                    onSelected = function()
                        ...
                        TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
                        Citizen.CreateThread(function()
                        ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Fabrication ~g~~h~en cours...", 'CHAR_CARSITE3', 7)
                        Citizen.Wait(7500)
                        ClearPedTasksImmediately(playerPed)
                        Citizen.Wait(100)
    ...
    Citizen.CreateThread(function()
        while true do  
            local wait = 750
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
                for k in pairs(Config.Position.FabricationKit) do 
                    local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
                    local pos = Config.Position.FabricationKit
                    local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, pos[k].x, pos[k].y, pos[k].z)
                    ...
                    if dist <= 2.0 then 
                        wait = 0
                        ESX.ShowHelpNotification(Config.TextFabricationKit) 
                        if IsControlJustPressed(1,51) then
                            fabrication()
                           end
                     end
...
--Vestiaire
                ...
                RageUI.IsVisible(mainMenuvestiaire,function() 
                  ...
                  RageUI.Checkbox("Prendre sa tenue Mécano", nil, service, {}, {
                    onChecked = function(index, items)
                        service = true
                        serviceon()
                        ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous venez de prendre votre ~g~tenue", 'CHAR_CARSITE3', 7)
                    end,
                    onUnChecked = function(index, items)
                        service = false
                        serviceoff()
                        ESX.ShowAdvancedNotification('Norauto', '~r~Notification', "Vous venez de ~r~ranger votre tenue", 'CHAR_CARSITE3', 7)
                    end
                })
    ...
    Citizen.CreateThread(function()
        while true do  
            local wait = 750
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
                for k in pairs(Config.Position.Vestiaire) do 
                    local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
                    local pos = Config.Position.Vestiaire
                    local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, pos[k].x, pos[k].y, pos[k].z)
...
AddEventHandler('message:benny', function(message, nom)
		ESX.ShowAdvancedNotification('Norauto', '~g~Message', '~s~De: ~r~'..nom..'\n~w~Message: ~y~'..message..'', 'CHAR_CARSITE3', 1)
end)
     ...
     if open then 
         ...
         while open do 
            RageUI.IsVisible(mainMenuBoss,function() 
              ...
              RageUI.Separator(VarColor.."↓ Gestion Comptabilité ↓") 
...
function ArgentRefresh(money)
    Mecano = ESX.Math.GroupDigits(money)
end

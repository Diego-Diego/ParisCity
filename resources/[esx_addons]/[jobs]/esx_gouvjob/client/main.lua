...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('gouv_wear'), uniform = grade}
	}
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'gouv')

						TriggerServerEvent('esx_service:disableService', 'gouv')
						TriggerEvent('esx_gouvjob:updateBlip')
...
function OpenGouvActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gouv_actions', {
		titlehead = 'Gouvernement',
		title = 'Interactions',
		align = 'top-left',
		elements = {
			...
			--{label = "Bracelet éléctronique", value = 'Ruby_Elec'}
	}}, function(data, menu)
		...
		elseif data.current.value == 'Ruby_Elec' then
			ESX.UI.Menu.CloseAll()
			TriggerEvent("Ruby_Elec:OpenMenu")
	...
	ESX.TriggerServerCallback('esx_gouvjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_gouvjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
	...
	ESX.TriggerServerCallback('esx_gouvjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_gouv', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_gouvjob:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_gouvjob:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_gouvjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_gouvjob:buyWeapon', function(bought)
				if bought then
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_gouvjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			titlehead = 'Inventaire',
			title    = 'Votre inventory',
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			local itemName = data.current.value
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_gouvjob:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gouv' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_gouvjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gouv' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_gouvjob:unrestrain')
AddEventHandler('esx_gouvjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'gouv' then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'gouv' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'gouv_actions') then
			if not Config.EnableESXService then
				OpenGouvActionsMenu()
			elseif playerInService then
				OpenGouvActionsMenu()
...
function createBlip(id)
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		SetBlipScale(blip, 1.2) -- set scale
		SetBlipAsShortRange(blip, true)
...
RegisterNetEvent('esx_gouvjob:updateBlip')
AddEventHandler('esx_gouvjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gouv' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'gouv' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_gouvjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_gouvjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_gouvjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'gouv')

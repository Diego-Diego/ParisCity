...
AddEventHandler('esx_gouvjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'gouv' then
		print(('esx_gouvjob: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_gouvjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_gouvjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_gouvjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_gouvjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'gouv' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_gouvjob:updateBlip', -1)
		end
	end
end)

RegisterNetEvent('esx_gouvjob:spawned')
AddEventHandler('esx_gouvjob:spawned', function()
	local xPlayer = ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('esx_gouvjob:forceBlip')
AddEventHandler('esx_gouvjob:forceBlip', function()
	TriggerClientEvent('esx_gouvjob:updateBlip', -1)
end)

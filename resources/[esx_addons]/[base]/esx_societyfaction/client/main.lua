...
function RefreshBussHUD()
	DisableSocietyFactionMoneyHUDElement()

	if ESX.PlayerData.faction.grade_name == 'boss' then
		EnableSocietyFactionMoneyHUDElement()
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	if ESX then
		if ESX.PlayerData.faction and ESX.PlayerData.faction.grade_name == 'boss' and 'society_' .. ESX.PlayerData.faction.name == society then
			UpdateSocietyMoneyHUDElement(money)
		end
	end
end)

function EnableSocietyFactionMoneyHUDElement()
	...
	TriggerEvent('esx_societyfaction:toggleSocietyHud', true)
end

function DisableSocietyFactionMoneyHUDElement()
	if ESX.GetConfig().EnableHud then
		ESX.UI.HUD.RemoveElement('society_money')
	end

	TriggerEvent('esx_societyfaction:toggleSocietyHud', false)
end
...
function UpdateSocietyMoneyHUDElement(money)
	...
	TriggerEvent('esx_societyfaction:setSocietyMoney', money)
end
...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_societyfaction:isBossFaction', function(isBoss)
		if isBoss then
				...
				if data.current.value == 'withdraw_society_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_societyfaction:withdrawMoneyFaction', society, amount)
						end
				...
				elseif data.current.value == 'deposit_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_societyfaction:depositMoneyFaction', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'wash_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_societyfaction:washMoneyFaction', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
function OpenEmployeeList(society)
	ESX.TriggerServerCallback('esx_societyfaction:getEmployeesFaction', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				...
				ESX.TriggerServerCallback('esx_societyfaction:setFaction', function()
					OpenEmployeeList(society)
				end, employee.identifier, 'civil', 0, 'fire')
			end
		end, function(data, menu)
			menu.close()
			OpenManageEmployeesMenu(society)
...
function OpenRecruitMenu(society)
	ESX.TriggerServerCallback('esx_societyfaction:getOnlinePlayersFaction', function(players)
		local elements = {}
...
function OpenManageGradesMenu(society)
	ESX.TriggerServerCallback('esx_societyfaction:getFaction', function(faction)
		...
		for i=1, #faction.grades, 1 do
			...
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, _U('money_generic', ESX.Math.GroupDigits(faction.grades[i].salary))),
				value = faction.grades[i].grade
			})
		end
...
AddEventHandler('esx_societyfaction:openBossMenu', function(society, close, options)

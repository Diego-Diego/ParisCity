...
RegisterNetEvent('crewPhone:blockPhone')
AddEventHandler('crewPhone:blockPhone', function()
  blockPhone = not blockPhone
end)

RegisterCommand('phone', function()
  if takePhoto ~= true and not blockPhone then
    TooglePhone()
  end
end, false)

RegisterKeyMapping('phone', 'Sortir/Ranger son téléphone', 'keyboard', 'F1')
  ...
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    ESX.TriggerServerCallback('crew-phone:phone-check', function(durum)
      if durum ~= nil then
        ...
        TriggerServerEvent('InteractSound_SV:PlayRing', 1.5, 'iphonex', 0.5)
...
AddEventHandler("gcPhone:stop-call-sound", function(phoneId)
   ...
   TriggerServerEvent('InteractSound_SV:StopRing', 2.0)
end)
      ...
      if durum ~= nil then
        ...
        if ShowNumberNotification == true then
          if message.transmitter == "police" or message.transmitter == "ambulance" then
            text = _U('new_message_hotline') .. message.transmitter
          else
            text = _U('new_message_normal') .. message.transmitter
          end
          for _,contact in pairs(contacts) do
            if contact.number == message.transmitter then
              text = _U('new_message_normal') .. contact.display
              break
            end
          end
        end
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      --ESX.ShowNotification('Upload de la photo..')
      ...
      --[[exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/808243651334766612/upKQV-EOxmuQDerQuOgaTXoT3rRnb1eE0mTk4wB6Nk02vYDFJ1i7bBSlBi2t0NudgqiD", "files[]", function(data)

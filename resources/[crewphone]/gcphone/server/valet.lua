...
ESX.RegisterServerCallback("gcPhone:getCars", function(a, b)
    ...
    MySQL.Async.fetchAll("SELECT plate, vehicle, state FROM owned_vehicles WHERE owner = @cid and type = @type", {["@cid"] = c.identifier, ["@type"] = "car"}, function(d)
        local e = {} for f, g in ipairs(d) do
            table.insert(e, {["garage"] = g["state"], ["plate"] = g["plate"], ["props"] = json.decode(g["vehicle"])})
        end;
        b(e)
    end)
end)
...
AddEventHandler("gcPhone:valet-car-set-outside", function(a)
    ...
    local c = ESX.GetPlayerFromId(b)
    if c then
        MySQL.Async.insert("UPDATE owned_vehicles SET state = @state WHERE plate = @plate", {["@plate"] = a, ["@state"] = 'Garage'})
    end
end)

        ...
        if tonumber(c) == tonumber(a) then
            ...
            if f <= 0 or f < tonumber(b) or tonumber(b) <= 0 then
                ...
                MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["@identifier"] = e.identifier}, function(g)
                    if g[1] then
                        local h = g[1].firstname .. " " .. g[1].lastname; MySQL.Async.fetchAll("INSERT INTO crew_phone_bank (type, identifier, price, name) VALUES (@type, @identifier, @price, @name)", {["@type"] = 1, ["@identifier"] = d.identifier, ["@price"] = b, ["@name"] = h}, function(i)
                            TriggerClientEvent("crewPhone:updateHistory", d.source)
                            TriggerEvent('MyCity_Core:TransfertsBancaires:Logs', 'Transfert d\'argent', d.getName() .. ' viens de transférer $' .. ESX.Math.GroupDigits(b) ..' à ' .. e.getName())
                        end)
                    end
                end)

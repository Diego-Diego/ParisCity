function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, faction, loadout, name, coords)
	...
	function self.setFaction(faction, grade)
		grade = tostring(grade)
		local lastFaction = json.decode(json.encode(self.faction))

		if ESX.DoesFactionExist(faction, grade) then
			local factionObject, factiongradeObject = ESX.Factions[faction], ESX.Factions[faction].grades[grade]
			...
			self.faction.grade_name = factiongradeObject.name
			self.faction.grade_label = factiongradeObject.label
			self.faction.grade_salary = factiongradeObject.salary

			if factiongradeObject.skin_male then
				self.faction.skin_male = json.decode(factiongradeObject.skin_male)
			else
				self.faction.skin_male = {}
			end

			if factiongradeObject.skin_female then
				self.faction.skin_female = json.decode(factiongradeObject.skin_female)
			else

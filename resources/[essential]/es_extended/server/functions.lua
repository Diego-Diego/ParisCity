...
function Core.SavePlayer(xPlayer, cb)
  MySQL.prepare(
    'UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `faction` = ?, `faction_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?',
    {json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.faction.name, xPlayer.faction.grade, xPlayer.group, json.encode(xPlayer.getCoords()),
...
function Core.SavePlayers(cb)
  ...
  if count > 0 then
    ...
    for i = 1, count do
      ...
      parameters[#parameters + 1] = {json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.faction.name, xPlayer.faction.grade, xPlayer.group,
...
function ESX.RefreshFactions()
  ...
  if not Factions then
    -- Fallback data, if no faction exist
    ESX.Factions['citizen'] = {label = 'Citoyen', grades = {['0'] = {grade = 0, label = 'RSA', salary = 200, skin_male = {}, skin_female = {}}}}
  else
...
function ESX.DoesFactionExist(faction, grade)
  grade = tostring(grade)

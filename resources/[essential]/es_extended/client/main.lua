...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.EnableHud then
		...
		local factionGradeLabel = ESX.PlayerData.faction.grade_label ~= ESX.PlayerData.faction.label and ESX.PlayerData.faction.grade_label or ''
		if factionGradeLabel ~= '' then factionGradeLabel = ' - '..factionGradeLabel end
		ESX.UI.HUD.RegisterElement('faction', #ESX.PlayerData.accounts, 0, factionTpl, {
			faction_label = ESX.PlayerData.faction.label,
			grade_label = factionGradeLabel
		})
	end
...
AddEventHandler('esx:setFaction', function(Faction)
	if Config.EnableHud then
		local gradeLabel = Faction.grade_label ~= Faction.label and Faction.grade_label or ''
		if gradeLabel ~= '' then gradeLabel = ' - '..gradeLabel end
